...
def find_urls(s):
	#makes sure it starts with http, includes at least one "." and at least two letters follow "."
    y = re.findall("http\S+\.\S+[a-zA-Z]{2,}", s)
    return(y)
    #Your code here
...
def grab_headlines():
	...
	#class for headlines from most read section 
	tags = soup(class_ = "view-most-read")
	#grabs text and returns in list of strings
	for i in tags:
...
def get_umsi_data():
	...
	title = []
	i = 0
	#13 pages to sift through
	for i in range(13):
		html = requests.get(url, headers={'User-Agent': 'SI_CLASS'})
		soup = BeautifulSoup(html.text, 'html.parser')
		#finding the faculty names and appending them into a list
		names = soup("div", "field-name-title")
		for k in names:
			k = k.text
			name.append(k)
		#finding the faculty positions and appending them to a list
		titles = soup("div", "field-name-field-person-titles")
		for v in titles:
			v = v.text
			title.append(v)
		#finding all the links to sift through 
		link = soup.find(class_ = "pager-next last")
		if i != 12:
			for url in link:
				url = "https://www.si.umich.edu" + url.get("href")
	#Return name as key and title as value in dictionary of strings
